...
export function IngredientNameInput({ value, onChange, disabled, placeholder = 'Назва', maxLength = 120 }) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const data = await apiFetch(
          `/ingredients/suggest?q=${encodeURIComponent(query)}&limit=10`,
  ...
  return (
    <div className="ingredientSuggest">
      ...
      />
      {open ? (
        <ul className="ingredientSuggestList" id={listId} role="listbox">
          {suggestions.map((name, index) => (
            <li key={name}>
              <button
                type="button"
                className={index === activeIndex ? 'ingredientSuggestItem ingredientSuggestItemActive' : 'ingredientSuggestItem'}
                role="option"
