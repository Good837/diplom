...
def _validate_image_upload(file: FileStorage | None) -> tuple[str, str]:
    ...
    normalized_ext = "jpg" if kind == "jpeg" else kind
    return normalized_ext, kind


def validate_uploaded_image(file: FileStorage | None) -> tuple[str, str]:
    return _validate_image_upload(file)
...
def upload_avatar():
    ...
    url = save_image(file, subdir="avatars", prefix="avatar", ext=ext)
...
def upload_recipe_image():
    file = request.files.get("file")
    ext, _kind = _validate_image_upload(file)
    url = save_image(file, subdir="recipes", prefix="recipe", ext=ext)
